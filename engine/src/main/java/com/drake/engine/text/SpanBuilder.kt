...
/**
 * 使用正则替换Span
 * @param span 每次匹配到字符串都会调用此函数, 返回一个指定的Span对象, 如果返回null則表示不替換
 */
fun CharSequence.replaceSpan(regex: String, span: (MatchResult) -> Any?): CharSequence {
 ...
 * @param span 每次匹配到字符串都会调用此函数, 返回一个指定的Span对象, 如果返回null則表示不替換
 */
fun CharSequence.replaceSpan(regex: Regex, span: (MatchResult) -> Any?): CharSequence {
    val spannable = SpannableString(this)
    regex.findAll(this).forEach {
        val range = it.range
        span(it)?.let { spanned ->
            spannable[range.first, range.last + 1] = spanned
        }
    }
    return spannable
}
 ...
 * @param span 每次匹配到字符串都会调用此函数, 返回一个指定的Span对象, 如果返回null則表示不替換
 */
fun CharSequence.replaceSpan(regex: String, replacement: CharSequence, span: ((MatchResult) -> Any?)? = null): CharSequence {
 ...
 * @param span 每次匹配到字符串都会调用此函数, 返回一个指定的Span对象, 如果返回null則表示不替換
 */
fun CharSequence.replaceSpan(regex: Regex, replacement: CharSequence, span: ((MatchResult) -> Any?)? = null): CharSequence {
    var offset = 0
    ...
    regex.findAll(this).forEach {
        ...
        spanBuilder.replace(it.range.first + offset, it.range.first + offset + matchLength, adjustReplacement)
        span?.invoke(it)?.let { spanned ->
            spanBuilder[it.range.first + offset, it.range.first + offset + replacementLength] = spanned
        }
...
class SpanBuilder() {
     ...
     *
     * @return the single [SpanBuilder] instance
     */
    ...
    fun setForegroundColorRes(@ColorRes color: Int): SpanBuilder {
        this.foregroundColor = app.resources.getColor(color)
        return this
    }
    ...
    fun setBackgroundColorRes(@ColorRes color: Int): SpanBuilder {
        this.backgroundColor = app.resources.getColor(color)
        return this
    }
     ...
     * @return the single [SpanBuilder] instance
     */
    fun setBullet(@IntRange(from = 0) gapWidth: Int): SpanBuilder {
     ...
     */
    @JvmOverloads
    fun setFontSize(@IntRange(from = 0) size: Int, isSp: Boolean = false): SpanBuilder {
        this.fontSize = size
     ...
     * @return the single [SpanBuilder] instance
     */
    fun setHorizontalAlign(alignment: Alignment): SpanBuilder {
     ...
     * @return the single [SpanBuilder] instance
     */
    fun setVerticalAlign(@Align align: Int): SpanBuilder {
     ...
     */
    @JvmOverloads
    fun appendImage(bitmap: Bitmap, @Align align: Int = ALIGN_BOTTOM): SpanBuilder {
        apply(typeImage)
     ...
     */
    @JvmOverloads
    fun appendImage(drawable: Drawable, @Align align: Int = ALIGN_BOTTOM): SpanBuilder {
        apply(typeImage)
     ...
     */
    @JvmOverloads
    fun appendImage(uri: Uri, @Align align: Int = ALIGN_BOTTOM): SpanBuilder {
        apply(typeImage)
        this.imageUri = uri
        this.alignImage = align
        return this
     ...
     */
    @JvmOverloads
    fun appendImage(@DrawableRes resourceId: Int, @Align align: Int = ALIGN_BOTTOM): SpanBuilder {
        apply(typeImage)
    ...
    private fun updateCharCharSequence() {
        ...
        if (verticalAlign != -1) {
            builder.setSpan(VerticalAlignSpan(verticalAlign), start, end, flag)
        }
        ...
        if (blurRadius != -1f) {
            builder.setSpan(MaskFilterSpan(BlurMaskFilter(blurRadius, style)), start, end, flag)
        }
        if (shader != null) {
    ...
    internal class SpaceSpan constructor(private val width: Int, color: Int = Color.TRANSPARENT) :
        ReplacementSpan() {
